---
layout: page
title: Links
permalink: /links/
description: 常用工具、项目与课题组链接整理
---

## 科研工具

### [Phonon Band Viewer]({{ '/tools/phonon/' | relative_url }})

本站内置的 `phonopy band.yaml` 可视化工具，可上传本地文件、查看色散关系，并直接播放原子振动模式。

### [Phonon Visualization](https://henriquemiranda.github.io/phononwebsite/phonon.html)

在线查看声子振动模式与相关可视化结果，适合快速核对模态示意。

### [Connected Papers](https://www.connectedpapers.com)

用于围绕一篇论文快速展开相关文献网络，适合找前后续工作与相近研究脉络。

### [Kiteflyingee/academic_prompts](https://github.com/Kiteflyingee/academic_prompts#高效阅读论文)

学术阅读与论文理解相关的 prompt 模板集合，可作为快速拆解论文、提炼问题和整理笔记的参考。

### [Use the CommKit](https://mitcommlab.mit.edu/nse/use-the-commkit/)

MIT NSE Communication Lab 的科研沟通指南集合，适合参考论文、报告、海报和申请材料的写作结构。

### [mattpocock/skills](https://github.com/mattpocock/skills)

面向真实工程工作的 AI agent skills 集合，适合参考其中的调试、TDD、需求梳理和工作流模板。

### [TBMethod](https://github.com/AlexanderZ11234/TBMethod)

紧束缚模型相关的方法与代码仓库，适合查阅实现思路和项目结构。

### [iLovePDF](https://www.ilovepdf.com/)

常用的 PDF 在线处理工具，适合合并、拆分、压缩和格式转换。

### [iLoveIMG](https://www.iloveimg.com/)

常用的图片在线处理工具，适合压缩、调整尺寸、裁剪和格式转换。

### [PDF to Markdown](https://pdf2md.morethan.io)

在线将 PDF 快速转换为 Markdown，适合整理论文、说明文档和后续知识库笔记。

### [Text Difference](https://www.jq22.com/textDifference)

在线文本差异对比工具，适合快速比较两段文本、代码或修改前后的内容。

### [去空行工具](http://www.esjson.com/delSpace.html)

在线删除文本空行的轻量工具，适合快速清理复制内容或 Markdown 草稿。

### [SwitchResX 4.8.0](https://www.waitsun.com/serial/switchresx-4-8-0)

macOS 屏幕分辨率与显示模式管理工具，适合外接显示器调试。

## 相关链接

### [CQUtug/CQULogo](https://github.com/CQUtug/CQULogo)

重庆大学校徽与相关视觉素材仓库，适合在制作报告、海报或网站内容时查找规范资源。

### [CQU-CTCMp](http://cquctcmp.com/)

重庆大学计算理论化学课题组主页，可用于查看课题组方向与成员信息。

## 写作与排版参考

### [LaTeX 公式参考](https://www.cnblogs.com/1024th/p/11623258.html)

常用 LaTeX 数学公式与符号写法参考。

### [MathJax 公式参考](https://freeopen.github.io/mathjax/#er-ju-zhen-shi-yong-can-kao)

Markdown 与 MathJax 数学公式排版参考。

### [Emoji Cheat Sheet](https://www.dute.org/emoji-cheat-sheet)

常用 Emoji 名称与代码速查。

### [Scribbr Citation Generator](https://www.scribbr.com/citation/generator/)

在线生成和整理常见格式的参考文献。

## 地图与资料

### [光污染地图](https://www.darkmap.cn/)

查看区域光污染情况，适合观星和天文摄影前做地点判断。

### [世界地图](https://bajiu.cn/ditu/)

在线世界地图与区域信息查询。

### [西北大学小型资料库](https://resour.nwu.icu)

课程教材与学习资料索引。
